<template>
   <div class="rated-film-div">
      ...
      <router-link :to="`/about/${movie.id}`">
        <div class="film-img"  >
            <img :src="imgPathDB+movie.poster_path" alt=""
            class="img">
...
export default {
    ...
    methods: {
           async zaprosPopularFilm () {
            const response =  await axios.get('https://api.themoviedb.org/3/movie/top_rated?api_key=4d6c6ca51430fa37ba6cc1c807a5c3e5&language=en-US&page=1')
            let arr = response.data.results
            for(let i=0;i<arr.length;i++){
                if(i<4){
                    this.popularFilms.push(arr[i])
                }
            }
